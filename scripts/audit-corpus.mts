...
const named: { page: string; pattern: RegExp; holds: (found: RegExpMatchArray) => string | null; what: string }[] = [
  // The two files an agent reads before anything else state their limits, and the numbers came from
  // constants nobody ties them to. Changing PER_DOMAIN_PER_HOUR would leave both files telling
  // agents a number that is no longer true, in a product whose whole argument is that you should
  // not lie to agents.
  ...['/agent-signup.md', '/agents.md'].map((page) => ({
    ...
    holds: (found: RegExpMatchArray) => {
      // Words and digits both, because the files were rewritten to digits on 2026-08-18 so a build
      // guard could read them, and this one then reported "says 5 per domain, code says 5".
      const asNumber = (told: string): number =>
        ({
