...
export const FIND_TOOL = {
  ...
  description:
    ...
    'Routing a sentence to a category is by far the weakest thing here, and the numbers below are the ones to plan around. Measured on 40 questions written by an agent with no access to this repository and no sight of the category list, six of them deliberately not about buying anything, labelled before the first run: it got 24 of the 40 right, said nothing on 6 it should have answered, sent 2 to the wrong category and answered 8 that it should have refused. Put another way: it gave an answer to 26 of the 40, and 10 of those answers were wrong. The likelier failure is a confident wrong answer rather than silence, and the wrong ones come from single words read out of context: deploying containers went to notifications on the word push, and a billing module inside the caller own repository went to payments. Older sets have been read and fixed against, so they no longer measure anything; this one has not. The tool reads English: the same 40 questions in Polish were answered 11 times with 6 placed. Name the category yourself when you know it.',
  inputSchema: {
    type: 'object',
    properties: {
