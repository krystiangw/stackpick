...
const FIND_TOOL = {
  ...
  description:
    ...
    'Routing a sentence to a category is by far the weakest thing here, and the number below is the one to plan around. Measured on 58 questions written before anybody read the rules and labelled before the first run: it answered 29 correctly, said nothing on 16 it should have answered, sent 9 to the wrong category and answered 4 that it should have refused. That set was written to press on words that belong to two categories at once, because ties are what this fails on; an earlier set without that emphasis scored 32 of 58, and both numbers are honest about the same tool. Put another way: it gave an answer to 38 of the 58, and 13 of those answers were wrong. Two older held-out sets have since been read and fixed against, so they no longer measure anything: a set stops being held out the moment somebody fixes what it found. Silence is the common failure and means we could not read the question rather than that the category is empty. Name the category yourself when you know it.',
  inputSchema: {
    type: 'object',
    properties: {
