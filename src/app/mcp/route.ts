...
export const FIND_TOOL = {
  ...
  description:
    ...
    'Routing a sentence to a category is by far the weakest thing here, and the number below is the one to plan around. Measured on 34 questions written before anybody read the rules and labelled before the first run, by a writer who had not seen the vocabulary or this repository: it answered 17 correctly, said nothing on 11 it should have answered, sent 3 to the wrong category and answered 3 that it should have refused. Put another way: it gave an answer to 18 of the 34, and 6 of those answers were wrong. Three older held-out sets have since been read and fixed against, so they no longer measure anything: a set stops being held out the moment somebody fixes what it found, and the error rate has stayed near half on every set written this way. Silence is the common failure and means we could not read the question rather than that the category is empty. Name the category yourself when you know it.',
  inputSchema: {
    type: 'object',
    properties: {
