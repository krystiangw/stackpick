...
export const FIND_TOOL = {
  ...
  description:
    ...
    'Routing a sentence to a category is by far the weakest thing here, and the numbers below are the ones to plan around. Measured on 40 questions written by an agent with no access to this repository and no sight of the category list, six of them deliberately not about buying anything and several deliberately niche, labelled before the first run: it got 21 of the 40 right, said nothing on 15 it should have answered, sent 1 to the wrong category and answered 3 that it should have refused. Put another way: it gave an answer to 14 of the 40, and 4 of those answers were wrong. It is deliberately quiet. One vocabulary word inside a long question no longer decides anything, because that is where the confident wrong answers came from, and on this set that traded three correct placements for ten fewer wrong ones: deploying containers no longer goes to notifications on the word push, and an events table in Postgres no longer goes to product analytics. Silence means we could not read the question, not that the category is empty. What still slips through is a question about your own code that happens to use two of our words: a billing module and a subscriptions module in your own repository still routes to payments, and merging overlapping time ranges goes to scheduling on booking and calendar. The tool reads English. Name the category yourself when you know it.',
  inputSchema: {
    type: 'object',
    properties: {
